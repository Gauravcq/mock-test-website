...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        function safeRender(text) {
            if (!text) return '';
            return containsHTML(text) ? text : escapeHtml(text);
        }
        ...
        function normalizeQuestion(raw) {
            ...
            if (typeof q.correctAnswer === 'string') {
                ...
                q.correctAnswer = { en: q.correctAnswer.en, hi: q.correctAnswer.hi || q.correctAnswer.en };
            } else if (typeof q.answer === 'string') {
                const letter = q.answer.trim().toUpperCase();
                if (letter.length === 1 && letter >= 'A' && letter <= 'D') {
                    const idx = letter.charCodeAt(0) - 65;
                    q.correctAnswer = q.options[idx] ? 
                        { en: q.options[idx].en, hi: q.options[idx].hi } : 
                        { en: q.answer, hi: q.answer };
                } else {
        ...
        if (typeof ALL_TESTS === 'undefined') {
            document.body.innerHTML = "<h1>Error: ALL_TESTS not found.</h1>";
            return;
        }

        const testInfo = ALL_TESTS.find(t => String(t.id) === testId);
        if (!testInfo) {
            document.body.innerHTML = `<h1>Error: Test "${testId}" not found.</h1>`;
            return;
        }
        ...
        try {
            ...
            } else throw new Error('No API questions');
        } catch (e) {
        ...
        if (!questions.length) {
            document.body.innerHTML = `<h1>No questions found for: ${testId}</h1><a href="index.html">Go Back</a>`;
            return;
        }
        ...
        function filterQuestions(category) {
            ...
            console.log('📊 States:', QD.questionStates.map((s,i) => `Q${i+1}:${s.resultCategory}`).join(', '));
        ...
        function showReviewQuestion(index) {
            ...
            q.options.forEach((opt, i) => {
                ...
                const optNorm = normalizeString(opt.en);
                const isCorrect = optNorm === correctNorm;
                const isUser = state.userAnswer && normalizeString(state.userAnswer) === optNorm;
                ...
                if (isCorrect && isUser) {
                    bgColor = '#dcfce7';
                    borderColor = '#22c55e';
                    icon = '<span style="color:#16a34a;margin-left:auto;font-weight:bold;">✓ Your Answer (Correct!)</span>';
                } else if (isCorrect) {
                    bgColor = '#dcfce7';
                    borderColor = '#22c55e';
                    icon = '<span style="color:#16a34a;margin-left:auto;font-weight:bold;">✓ Correct Answer</span>';
                } else if (isUser) {
                    bgColor = '#fee2e2';
                    borderColor = '#ef4444';
                    icon = '<span style="color:#dc2626;margin-left:auto;font-weight:bold;">✗ Your Answer</span>';
                }

                optionsHTML += `
                    <div style="display:flex;align-items:center;padding:14px 16px;margin:10px 0;background:${bgColor};border:2px solid ${borderColor};border-radius:10px;">
                        <span style="font-weight:600;margin-right:12px;min-width:24px;">${letter}.</span>
            ...
            if (state.userAnswer === null) {
                statusNote = `
                    <div style="background:#fef3c7;border:2px solid #f59e0b;border-radius:10px;padding:14px;margin-top:16px;color:#92400e;font-weight:500;">
                        ⚠️ This question was NOT ATTEMPTED. The correct answer is highlighted in green.
                    </div>
                `;
            }
            ...
            if (reviewQuestionCard) {
                reviewQuestionCard.innerHTML = `
                    <div style="font-size:17px;line-height:1.7;margin-bottom:20px;">
                        <strong style="color:#4f46e5;">Q${item.index + 1}.</strong> ${safeRender(qText)}
                    </div>
                    ${optionsHTML}
                    ${statusNote}
                `;
            ...
            if (reviewSolutionText) {
                reviewSolutionText.innerHTML = explText ? safeRender(explText) : '<em style="color:#9ca3af;">No explanation available.</em>';
            }
            ...
            if (reviewPaletteClean) {
                ...
                QD.reviewQuestionList.forEach((it, idx) => {
                    ...
                    btn.style.cssText = 'width:36px;height:36px;margin:3px;border:none;border-radius:6px;cursor:pointer;font-weight:600;';
                    
                    if (st.resultCategory === 'correct') btn.style.background = '#22c55e';
                    else if (st.resultCategory === 'incorrect') btn.style.background = '#ef4444';
                    else btn.style.background = '#fbbf24';
                    
                    btn.style.color = 'white';
                    if (idx === QD.currentReviewIndex) btn.style.boxShadow = '0 0 0 3px #4f46e5';
        ...
        function handleTabClick(e) {
            ...
            console.log('📋 Found:', window.QUIZ_DATA.reviewQuestionList.length);
        ...
        function initQuiz() {
            ...
            function showSubmitModal() {
                ...
                if (submissionStatsEl) {
                    submissionStatsEl.innerHTML = `
                        <div style="margin:8px 0;">Answered: <strong>${answered}/${questions.length}</strong></div>
                        <div style="margin:8px 0;">Unanswered: <strong>${questions.length - answered}/${questions.length}</strong></div>
                    `;
                }
            ...
            function submitQuiz() {
                ...
                console.log('✅ Results:', { correct, incorrect, unattempted });
                console.log('📊 Categories:', QD.questionStates.map((s,i) => `Q${i+1}:${s.resultCategory}`).join(', '));
                ...
                if (reviewArea) {
                    reviewArea.innerHTML = `
                        <div style="margin-bottom:20px;">
                            <h3 style="margin-bottom:8px;">${QD.testInfo.title || 'Test'}</h3>
                            <p style="color:#6b7280;">Questions: ${questions.length} | Max: ${questions.length * 2} marks</p>
                        </div>
                    ...
                    statsArea.innerHTML = `
                        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(100px,1fr));gap:12px;margin-top:20px;">
                            <div style="background:#f0fdf4;padding:20px;border-radius:12px;text-align:center;">
                                ...
                                <div style="color:#6b7280;font-size:13px;margin-top:4px;">Score</div>
                            </div>
                            <div style="background:#dcfce7;padding:20px;border-radius:12px;text-align:center;">
                                ...
                                <div style="color:#6b7280;font-size:13px;margin-top:4px;">Correct</div>
                            </div>
                            <div style="background:#fee2e2;padding:20px;border-radius:12px;text-align:center;">
                                ...
                                <div style="color:#6b7280;font-size:13px;margin-top:4px;">Incorrect</div>
                            </div>
                            <div style="background:#fef3c7;padding:20px;border-radius:12px;text-align:center;">
                                ...
                                <div style="color:#6b7280;font-size:13px;margin-top:4px;">Skipped</div>
                            </div>
                            <div style="background:#f3f4f6;padding:20px;border-radius:12px;text-align:center;">
                                ...
                                <div style="color:#6b7280;font-size:13px;margin-top:4px;">Time</div>
                            </div>
                            <div style="background:#eff6ff;padding:20px;border-radius:12px;text-align:center;">
                                ...
                                <div style="color:#6b7280;font-size:13px;margin-top:4px;">Accuracy</div>
                            </div>
                        </div>
                    `;
                }
            ...
            function showQuestion(index) {
                ...
                q.options.forEach((opt, i) => {
                    ...
                    optionsHTML += `
                        <label style="display:flex;align-items:center;padding:14px 16px;margin:10px 0;background:#f8fafc;border:2px solid #e2e8f0;border-radius:10px;cursor:pointer;transition:all 0.2s;">
                            <input type="radio" name="option" value="${escapeHtml(value)}" ${checked} style="margin-right:14px;width:18px;height:18px;">
                            <span style="font-weight:600;margin-right:10px;">${letter}.</span>
                            <span>${safeRender(text)}</span>
                        </label>
                    `;
                });
                ...
                if (questionArea) {
                    questionArea.innerHTML = `
                        <div style="font-size:17px;line-height:1.7;margin-bottom:24px;">
                            <strong style="color:#4f46e5;">Q${index + 1}.</strong> ${safeRender(qText)}
                        </div>
            ...
            function updateNav() {
                ...
                if (nextBtn) nextBtn.textContent = QD.currentQuestionIndex === questions.length - 1 ? 'Submit Test' : 'Save & Next';
            }
