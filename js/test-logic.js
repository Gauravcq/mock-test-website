...
(function () {
  ...
  const SECURITY = {
    ...
    detectTabSwitch() {
      ...
      document.addEventListener("visibilitychange", () => {
        if (
          document.hidden &&
          window.QUIZ_DATA?.isQuizStarted &&
          !window.QUIZ_DATA?.isSubmitted
        ) {
          tabSwitchCount++;
          ...
          if (tabSwitchCount >= maxTabSwitches) {
            this.showWarning(
              `⚠️ WARNING: Tab switch ${tabSwitchCount}/${maxTabSwitches}! Test may be auto-submitted.`,
              "error",
            );
            ...
            this.showWarning(
              `⚠️ Tab switch detected! (${tabSwitchCount}/${maxTabSwitches})`,
              "warning",
            );
          }

          window._originalConsole?.warn?.(
            `Tab switch: ${tabSwitchCount}/${maxTabSwitches}`,
          );
        }
      });
    ...
    handleDevToolsOpen() {
      if (
        !this._devToolsWarningShown &&
        document.body.classList.contains("exam-mode")
      ) {
        this._devToolsWarningShown = true;
        this.showWarning(
          "🛠️ Developer tools detected! This is being logged.",
          "error",
        );
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    function textsMatch(a, b) {
      if (!a || !b) return false;
      const normalize = (str) =>
        String(str).normalize("NFKC").replace(/\s+/g, " ").trim().toLowerCase();
    ...
    if (!testId) {
      document.body.innerHTML =
        '<div style="text-align:center;padding:50px;"><h1>Error: No Test ID</h1><a href="index.html">Go Back</a></div>';
      return;
    }
    if (typeof ALL_TESTS === "undefined") {
      document.body.innerHTML =
        '<div style="text-align:center;padding:50px;"><h1>Error: ALL_TESTS missing</h1></div>';
      return;
    }
    const testInfo = ALL_TESTS.find((t) => String(t.id) === testId);
    if (!testInfo) {
    ...
    let questionsSource = "";
    if (
      typeof QUESTIONS_DATABASE !== "undefined" &&
      QUESTIONS_DATABASE[testId]
    ) {
        ...
        if (response?.success && response?.data?.questions?.length) {
          ...
          if (apiQuestions[0]?.correctAnswer) {
            ...
            if (typeof QUESTIONS_DATABASE !== "undefined") {
              ...
              if (localData) {
                const localQuestions = Array.isArray(localData)
                  ? localData
                  : localData.questions || [];
    ...
    const hasFullMockSection =
      testInfo.section === "fullmock" || testInfo.type === "fullmock";
    ...
    const isFullMockTest =
      hasFullMockInId ||
      hasFullMockSection ||
      (hasFullMockCount && !hasMultipleSubjects);
    ...
    console.log(
      `✅ Loaded ${window.QUIZ_DATA.questions.length} questions from ${questionsSource}`,
    );
        ...
        if (q.subject !== currentSubject) {
          if (currentSubject !== null) {
            const newSection = {
              ...
              label:
                subjectLabels[currentSubject] || currentSubject.toUpperCase(),
    ...
    function findCorrectOptionIndex(q, lang) {
      ...
      for (let i = 0; i < q.options.length; i++) {
        ...
        if (
          opt.en === correctText ||
          opt[lang] === correctText ||
          opt.en?.trim() === correctText ||
          opt[lang]?.trim() === correctText ||
          textsMatch(opt.en, correctText) ||
          textsMatch(opt[lang], correctText)
        ) {
          return i;
        }
      }
    ...
    function filterQuestions(category) {
      ...
      if (cat === "correct")
        return all.filter((x) => x.state?.resultCategory === "correct");
      if (cat === "incorrect")
        return all.filter((x) => x.state?.resultCategory === "incorrect");
      if (cat === "unattempted")
        return all.filter((x) => x.state?.resultCategory === "unattempted");
      if (cat === "marked for review")
        return all.filter((x) => x.state?.markedForReview);
      return all;
    }
    ...
    function showReviewQuestion(index) {
      ...
      if (
        !QD.reviewQuestionList.length ||
        index < 0 ||
        index >= QD.reviewQuestionList.length
      )
        return;
      ...
      if (reviewNextBtn)
        reviewNextBtn.disabled = index >= QD.reviewQuestionList.length - 1;
      if (window.MathJax)
        try {
          MathJax.typeset();
        } catch (e) {}
    ...
    function handleTabClick(e) {
      ...
      [resultTabsContainer, reviewTabsContainer].forEach((nav) => {
        if (nav)
          nav.querySelectorAll("a").forEach((a) => {
            a.classList.toggle(
              "active",
              a.textContent.toLowerCase().trim() === cat,
            );
          });
      });
      if (cat === "overview") {
      ...
      if (window.QUIZ_DATA.reviewQuestionList.length > 0) {
        ...
        if (reviewQuestionTitle)
          reviewQuestionTitle.textContent = "No questions";
    ...
    if (reviewNextBtn) {
      reviewNextBtn.onclick = () => {
        if (
          window.QUIZ_DATA.currentReviewIndex <
          window.QUIZ_DATA.reviewQuestionList.length - 1
        ) {
          showReviewQuestion(window.QUIZ_DATA.currentReviewIndex + 1);
        }
      };
    ...
    function initQuiz() {
      ...
      const duration =
        QD.testInfo.duration || (QD.testInfo.section === "fullmock" ? 60 : 25);
      ...
      if (QD.isFullMock) {
        ...
        console.log(
          `⏱️ Full Mock Timer: ${duration} minutes (shared across all sections)`,
        );
      ...
      function updateSectionTabs() {
        ...
        tabsContainer.querySelectorAll(".section-tab").forEach((tab) => {
          const section = QD.sections.find(
            (s) => s.subject === tab.dataset.section,
          );
          if (!section) return;
      ...
      function showSubmitModal() {
        ...
        const answered = QD.questionStates.filter(
          (s) => s.status === "answered",
        ).length;
        const notAnswered = QD.questionStates.filter(
          (s) => s.status === "not-answered",
        ).length;
        const notVisited = QD.questionStates.filter(
          (s) => s.status === "not-visited",
        ).length;
        const marked = QD.questionStates.filter(
          (s) => s.markedForReview,
        ).length;
      ...
      function startTimer() {
        ...
        QD.timerInterval = setInterval(() => {
          ...
          if (QD.isFullMock) {
            ...
            if (
              currentSubject &&
              typeof QD.sectionTimeRemaining[currentSubject] === "number"
            ) {
              QD.sectionTimeRemaining[currentSubject] = Math.max(
                0,
                QD.sectionTimeRemaining[currentSubject] - 1,
              );
            }
          }
      ...
      function submitQuiz() {
        ...
        QD.questionStates.forEach((state, i) => {
          ...
          if (state.userAnswer !== null && state.userAnswer !== "") {
            const isMatch =
              textsMatch(state.userAnswer, correctText) ||
        ...
        try {
          // ✅ Enhanced attempt payload with proper question mapping
          const normalizedQuestions = questions.map((q, index) => ({
            id: `${testId}_${index}`, // Unique ID per test and question
            originalIndex: index,
          ...
          try {
            ...
            console.log(
              "✅ Saved to testResults map for testId:",
              String(QD.testInfo.id || testId),
            );
          } catch (e) {
        ...
        if (window.ExamAxisAPI?.isLoggedIn()) {
          ...
          // ✅ Create proper answer mapping with test-specific question IDs
          const answersObj = {};
          QD.questionStates.forEach((state, i) => {
            const questionId = `${testId}_${i}`; // Match the question ID format
            answersObj[questionId] = state.userAnswer;
            // Also store by index for backward compatibility
            answersObj[i] = state.userAnswer;
          });

          // ✅ CRITICAL: Prepare questions snapshot with unique test-specific IDs
          const questionsSnapshot = questions.map((q, index) => ({
            id: `${testId}_${index}`, // Unique per test
            originalIndex: index,
            testId: String(QD.testInfo.id || testId),
            subject: q.subject || subjectName || "General",
            question: q.question,
            ...
            correctAnswer: q.correctAnswer,
            explanation: q.explanation || "",
            // Store original question for debugging
            _originalQuestion: q,
          }));

          const attemptData = {
            testId: String(QD.testInfo.id || testId),
            examType: detectExamType(testId) || "CGL",
            subject: subjectName || detectSubjectFromTest(testId) || "General",
            score: Number(score.toFixed(2)),
            ...
            timeTaken: mins,
            answers: answersObj,
            // ✅ Send questions snapshot with test-specific IDs
          ...
          console.log("📤 Saving attempt with enhanced data:", {
          ...
          // ✅ Save to both backend and enhanced localStorage
          Promise.all([
            ExamAxisAPI.saveTestAttempt(attemptData),
            saveEnhancedLocalStorage(attemptData),
          ])
            .then(([backendResult, localResult]) => {
              console.log("✅ Save attempt results:", {
          ...
          console.warn("⚠️ User not logged in, attempt saved locally only");
          ...
          saveEnhancedLocalStorage(localAttemptData);
        }

        // ✅ Helper function to detect exam type from test ID
        ...
        // ✅ Helper function to detect subject from test ID
        function detectSubjectFromTest(testId) {
          const id = String(testId).toLowerCase();
          if (id.includes("math")) return "Mathematics";
          if (id.includes("english")) return "English";
          if (id.includes("reasoning")) return "Reasoning";
          if (id.includes("gk") || id.includes("general"))
            return "General Knowledge";
          if (id.includes("full") || id.includes("mock"))
            return "Full Mock Test";
          return "General";
        }

        // ✅ Enhanced localStorage save with test isolation
        function saveEnhancedLocalStorage(attemptData) {
          try {
            // Save individual attempt
            const attemptKey = `attempt_${attemptData.testId}_${Date.now()}`;
            localStorage.setItem(attemptKey, JSON.stringify(attemptData));

            // Update test-specific results map
            const testResults = JSON.parse(
              localStorage.getItem("testResults") || "{}",
            );
            testResults[attemptData.testId] = {
              ...attemptData,
              lastAttemptKey: attemptKey,
            };
            localStorage.setItem("testResults", JSON.stringify(testResults));

            // Update general testResult for backward compatibility
            localStorage.setItem("testResult", JSON.stringify(attemptData));

            console.log("✅ Enhanced local storage saved:", {
              attemptKey,
              testId: attemptData.testId,
            });

            return { success: true, attemptKey };
          } catch (error) {
            console.error("❌ Enhanced local storage save failed:", error);
      ...
      function updatePalette() {
        ...
        questionPalette
          ...
          .forEach((btn, btnIndex) => {
            const globalIndex = QD.isFullMock
              ? parseInt(btn.dataset.globalIndex)
              : btnIndex;
            const st = QD.questionStates[globalIndex];
            btn.className = "palette-btn";
            if (st.userAnswer && st.markedForReview)
            ...
            else if (st.status === "not-answered")
              btn.classList.add("not-answered");
            else btn.classList.add("not-visited");
            if (globalIndex === QD.currentQuestionIndex)
      ...
      function updateNav() {
        ...
        if (nextBtn)
          nextBtn.textContent =
            QD.currentQuestionIndex === questions.length - 1
              ? "Submit"
              : "Save & Next";
      }
      ...
      if (markReviewBtn) {
        markReviewBtn.onclick = () => {
          QD.questionStates[QD.currentQuestionIndex].markedForReview =
            !QD.questionStates[QD.currentQuestionIndex].markedForReview;
          saveAnswer();
          if (QD.currentQuestionIndex < questions.length - 1) {
            showQuestion(QD.currentQuestionIndex + 1);
      ...
      console.log(
        "⏱️ Timer Duration:",
        QD.isFullMock
          ? `${QD.sharedTimeRemaining} seconds (shared)`
          : "Per section",
      );
      ...
      switch (e.key) {
        ...
        case "4":
          ...
          if (options[optionIndex]) {
            ...
            options[optionIndex].dispatchEvent(
        ...
        case "c":
        case "C":
          if (!e.ctrlKey && !e.metaKey && clearResponseBtn)
